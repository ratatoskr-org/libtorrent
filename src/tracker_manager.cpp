...
namespace libtorrent::aux {
	...
	std::size_t tracker_manager::http_pool_key_hash::operator()(http_pool_key const& k) const
	{
		...
		boost::hash_combine(ret, std::hash<void*>{}(static_cast<void*>(k.bind.get())));
		...
		boost::hash_combine(ret, std::hash<bool>{}(k.is_stop));
		return ret;
	}
	...
	tracker_manager::http_pool_key tracker_manager::make_pool_key(tracker_request const& req)
	{
		...
		// so they don't block (or get blocked by) normal keep-alive announces.
		// In steady state they share the same connection as regular announces.
		key.is_stop = m_abort && (req.event == event_t::stopped);
...
#endif
		aux::proxy_settings ps(m_settings);
	...
	void tracker_manager::remove_request(aux::http_tracker_connection const* c)
	{
		...
		m_stats_counters.set_value(counters::num_queued_tracker_announces,
			std::int64_t(m_http_conns.size()) - m_num_started_http);
	}
	...
	void tracker_manager::abort_all_requests(bool all)
	{
		...
		for (auto const& e : m_http_conns)
		{
			...
			close_http_connections.push_back(e.conn);
